...
bool RCSwitch::receiveProtocol1(unsigned int changeCount){
        ...
        #define dt delayTolerance

        
        for (int i = 1; i<changeCount ; i=i+2) {
            
                if (RCSwitch::timings[i] > delay-dt
                        && RCSwitch::timings[i] < delay+dt
                        && RCSwitch::timings[i+1] > delay*3-dt
                        && RCSwitch::timings[i+1] < delay*3+dt) {
                    code = code << 1;

                } else if (RCSwitch::timings[i] > delay*3-dt
                        && RCSwitch::timings[i] < delay*3+dt
                        && RCSwitch::timings[i+1] > delay-dt
                        && RCSwitch::timings[i+1] < delay+dt) {
